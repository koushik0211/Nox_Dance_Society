...
import { useIsMobile } from '../../hooks/useIsMobile';
// useAuth is no longer needed here as we are not checking roles on this page
...
const ManageAuditionsPage = () => {
    ...
    const handleRejectPending = async () => {
        ...
        if (pendingCount === 0) {
            alert("There are no candidates with 'Pending' status to reject.");
            return;
        }
        if (window.confirm(`Are you sure you want to change all ${pendingCount} "Pending" entries to "Not Selected"?`)) {
            try {
                const response = await auditionService.rejectAllPending();
                alert(response.data.message || "Bulk rejection successful.");
                fetchAuditions();
            } catch (err) {
                const errorMessage = err.response?.data?.message || "Failed to perform bulk rejection.";
                setError(errorMessage);
                alert(`Error: ${errorMessage}`);
            }
        }
    ...
    const notSelectedCount = useMemo(() => auditions.filter(a => a.status === 'Not Selected').length, [auditions]);
    const pendingCount = useMemo(() => auditions.filter(a => a.status === 'Pending').length, [auditions]);
    const hasSlottedAuditions = useMemo(() => auditions.some(a => a.timeSlot && a.timeSlot !== 'Unassigned'), [auditions]);
    ...
    const renderContent = (data) => isMobile ? <MobileCardList data={data} /> : <DesktopTable data={data} />;
    ...
    const ActionButtons = ({ isDropdown = false }) => (
        <>
            ...
            <button onClick={() => { handleRejectPending(); setIsMobileMenuOpen(false); }} className={isDropdown ? "dropdown-action-button reject" : "action-button reject-all"} disabled={pendingCount === 0}>
    ...
    return (
        <div className="manage-page">
            ...
            {viewMode === 'list' ? renderContent(processedAuditions.list) : (
                <div className="slots-view-container">
                    {processedAuditions.sortedSlotKeys.length > 0 ? (
                        processedAuditions.sortedSlotKeys.map((slot, slotIndex) => (
                            <div key={slot} className="slot-group">
                                <h3 className="slot-group-title">
                                    Slot {slotIndex + 1}: {slot} <span>({processedAuditions.slots[slot].length} candidates)</span>
                        ...
                        <p className="no-data-cell">No entries have been slotted yet.</p>
                    )}
                </div>
            )}
